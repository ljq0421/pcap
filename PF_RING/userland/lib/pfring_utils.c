...
int pfring_parse_pkt(u_char *data, struct pfring_pkthdr *hdr, u_int8_t level /* L2..L4, 5 (tunnel) */,
  ...
  //printf("pfring_utils.c  pfring_parse_pkt\n");//1
...
static char *etheraddr2string(const u_char *ep, char *buf) {
  ...
  //printf("pfring_utils.c  etheraddr2string\n");
...
static char *proto2str(u_short proto) {
  ...
  //printf("pfring_utils.c  proto2str\n");
...
int pfring_print_parsed_pkt(char *buff, u_int buff_len, const u_char *p, const struct pfring_pkthdr *h) {
  ...
  //printf("pfring_utils.c  pfring_print_parsed_pkt\n");
  ...
  if (h->extended_hdr.parsed_pkt.eth_type == 0x0800 /* IPv4*/ || h->extended_hdr.parsed_pkt.eth_type == 0x86DD /* IPv6*/) {
   ...
   if(h->extended_hdr.parsed_pkt.eth_type == 0x0800 ){// IPv4{
      buff_used += snprintf(&buff[buff_used], buff_len - buff_used,
        "%s*%d", intoa(h->extended_hdr.parsed_pkt.ipv4_src), h->extended_hdr.parsed_pkt.l4_src_port);
      buff_used += snprintf(&buff[buff_used], buff_len - buff_used,
        ">%s*%d|", intoa(h->extended_hdr.parsed_pkt.ipv4_dst), h->extended_hdr.parsed_pkt.l4_dst_port);
    } else {
      buff_used += snprintf(&buff[buff_used], buff_len - buff_used,
        "%s*%d",    in6toa(h->extended_hdr.parsed_pkt.ipv6_src), h->extended_hdr.parsed_pkt.l4_src_port);
      buff_used += snprintf(&buff[buff_used], buff_len - buff_used,
        ">%s*%d|", in6toa(h->extended_hdr.parsed_pkt.ipv6_dst), h->extended_hdr.parsed_pkt.l4_dst_port);
    }

    //buff_used += snprintf(&buff[buff_used], buff_len - buff_used,"[l3_proto=%s]", proto2str(h->extended_hdr.parsed_pkt.l3_proto));
    ...
    //buff_used += snprintf(&buff[buff_used], buff_len - buff_used,"[hash=%u]",h->extended_hdr.pkt_hash);
	...
	//int len1=strlen(etheraddr2string(&p[h->extended_hdr.parsed_pkt.offset.l3_offset+8], buf1));
	//int len2=strlen(etheraddr2string(&p[h->extended_hdr.parsed_pkt.offset.l3_offset+18], buf2));
	int len3=strlen(intoa(ntohl(*((u_int32_t *) &p[h->extended_hdr.parsed_pkt.offset.l3_offset+14]))));
	int len4=strlen(intoa(ntohl(*((u_int32_t *) &p[h->extended_hdr.parsed_pkt.offset.l3_offset+24]))));
  	//printf("len3=%d,len4=%d\n",len3,len4);
	//printf("yuan ip:%s\n",intoa(ntohl(*((u_int32_t *) &p[h->extended_hdr.parsed_pkt.offset.l3_offset+14]))));
	//printf("mudi ip:%s\n",intoa(ntohl(*((u_int32_t *) &p[h->extended_hdr.parsed_pkt.offset.l3_offset+24]))));
	//buff_used+=snprintf(&buff[buff_used],len1+len2+7,"[%s -> %s] ",etheraddr2string(&p[h->extended_hdr.parsed_pkt.offset.l3_offset+8], buf1),etheraddr2string(&p[h->extended_hdr.parsed_pkt.offset.l3_offset+18], buf2));
	buff_used+=snprintf(&buff[buff_used],len3+7,"%s>",intoa(ntohl(*((u_int32_t *) &p[h->extended_hdr.parsed_pkt.offset.l3_offset+14]))));
	buff_used+=snprintf(&buff[buff_used],len4+4,"%s|",intoa(ntohl(*((u_int32_t *) &p[h->extended_hdr.parsed_pkt.offset.l3_offset+24]))));
    }

  }else if(h->extended_hdr.parsed_pkt.eth_type == 0x8035 /* RARP */) {
    buff_used += snprintf(&buff[buff_used], buff_len - buff_used, "RARP|");
    if (buff_len >= h->extended_hdr.parsed_pkt.offset.l3_offset+30) { 
	int len3=strlen(intoa(ntohl(*((u_int32_t *) &p[h->extended_hdr.parsed_pkt.offset.l3_offset+14]))));
	int len4=strlen(intoa(ntohl(*((u_int32_t *) &p[h->extended_hdr.parsed_pkt.offset.l3_offset+24]))));
 	buff_used+=snprintf(&buff[buff_used],len3+7,"%s>",intoa(ntohl(*((u_int32_t *) &p[h->extended_hdr.parsed_pkt.offset.l3_offset+14]))));
...
int pfring_print_pkt(char *buff, u_int buff_len, const u_char *p, u_int len, u_int caplen) {
  ...
  printf("pfring_utils.c  pfring_print_pkt\n");
...
static int __pfring_set_if_promisc(const char *device, int set_promisc) {
  ...
  printf("pfring_utils.c  __pfring_set_if_promisc\n");
...
int pfring_set_if_promisc(const char *device, int set_promisc) {
  ...
  printf("pfring_utils.c  pfring_set_if_promisc\n");
...
int pfring_set_promisc(pfring *ring, int set_promisc) {
  ...
  //printf("pfring_utils.c  pfring_set_promisc\n");
...
char* pfring_format_numbers(double val, char *buf, u_int buf_len, u_int8_t add_decimals) {
  ...
  //printf("pfring_utils.c  pfring_format_numbers\n");//Leaving ...
...
int pfring_get_mtu_size(pfring* ring) {
  ...
  //printf("pfring_utils.c  pfring_get_mtu_size\n");
...
int pfring_parse_bpf_filter(char *filter_buffer, u_int caplen,
  ...
  printf("pfring_utils.c  pfring_parse_bpf_filter\n");
...
u_int32_t pfring_bpf_filter(void *bpf_insn, u_char *buffer, u_int32_t caplen, u_int32_t len) {
  ...
  printf("pfring_utils.c  pfring_bpf_filter\n");
