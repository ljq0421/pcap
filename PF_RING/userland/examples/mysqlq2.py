...
cursor1.execute("select uuid,user_id from instances where vm_state='active'")
...
#print mail
for i in range(len(cds1)):
	cursor3.execute('insert into ins_user (user_id,uuid,float_ip,ip,mail,tap) values("'+user_id[i]+'","'+uuid[i]+'","'+ip1[i]+'","'+ip2[i]+'","'+mail[i]+'","'+dev[i]+'")')
