...
static inline u_int64_t get_next_slot_offset(struct pf_ring_socket *pfr, u_int64_t off)
{
  ...
  real_slot_size = pfr->slot_header_len + hdr->caplen;//188+caplen

  /* padding at the end of the packet (magic number added on insert) */
  real_slot_size += sizeof(u_int16_t); /* RING_MAGIC_VALUE *///+2

  /* Align slot size to 64 bit */
  real_slot_size = ALIGN(real_slot_size, sizeof(u_int64_t));//与64bit对齐，8byte

  if((off + real_slot_size + pfr->slots_info->slot_len) > (pfr->slots_info->tot_mem - sizeof(FlowSlotInfo)))
  //在get_next_slot_offset中，off是insert_off，写指针，如果计算之后的偏移在分配的总内存之外，返回0：环状缓冲回到头部
    return 0;
...
static inline int check_free_ring_slot(struct pf_ring_socket *pfr)
{
  ...
  if(pfr->slots_info->insert_off == remove_off) {
    ...
    if(queued_pkts >= pfr->slots_info->min_num_slots)//排在队列中的数据包比slot数目多
      return(0); /* Memory is full */

  } else if(pfr->slots_info->insert_off < remove_off) {
    //写指针在读指针之前，检查是否有足够的空间存放一个新的数据包
...
static inline int ring_insert(struct sock *sk)
{
  ...
  atomic_inc(&ring_table_size);//表示环的数量
				    ...
				    void *raw_data, uint raw_data_len)
{
  ...
  ring_bucket = get_slot(pfr, off);//off=insert_off，根据insert_off获得当前slot位置

  if(skb != NULL) {
    /* Copy skb data */

    hdr->caplen = min_val(hdr->caplen, pfr->bucket_len - offset);//返回hdr->caplen和pfr->bucket_len - offset中较小的那个
  ...
  pfr->slots_info->insert_off = get_next_slot_offset(pfr, off);//off=insert_off
			    ...
			    u_int32_t num_rx_channels)
{
  ...
  if(atomic_read(&ring_table_size) == 0)
    return(0);//有一个环就可以捕获数据包，如果没有就停止，return
  ...
  if(quick_mode) {
	...
	room_available |= add_skb_to_ring(skb, real_skb, pfr, &hdr, is_ip_pkt,
					  displ, channel_id, num_rx_channels);//|=按位或，然后赋值
		       ...
		       )
{
...
#if(LINUX_VERSION_CODE < KERNEL_VERSION(3,8,0))
  if(!capable(CAP_NET_ADMIN))//是否有权创建PF_RING套接口
#else
  if(!ns_capable(net->user_ns, CAP_NET_RAW))
#endif
    return -EPERM;

  if(sock->type != SOCK_RAW)//仅支持sock_raw类型的套接字
  ...
  sk = sk_alloc(net, PF_INET, GFP_KERNEL, &ring_proto);//sk_alloc从系统中的高速缓存空间中申请一个地址族为PF_RING的sock空间
#else
  ...
  sock->ops = &ring_ops;
  sock_init_data(sock, sk);//初始化sock结构的部分成员

  ring_sk(sk) = (struct pf_ring_socket *) kmalloc(sizeof(*pfr), GFP_KERNEL);//为ring_opt分配空间，初始化slots等待队列

  if(!(pfr = ring_sk(sk)))
    goto free_sk;

  memset(pfr, 0, sizeof(*pfr));//pf_ring_socket *pfr
  pfr->sk = sk;
  pfr->ring_shutdown = 0;
  pfr->ring_active = 0;	/* We activate as soon as somebody waits for packets *///只要有数据包在队列中等，ring就被激活了
  pfr->num_rx_channels = UNKNOWN_NUM_RX_CHANNELS;
  pfr->channel_id_mask = RING_ANY_CHANNEL;
  pfr->bucket_len = DEFAULT_BUCKET_LEN;//128
  ...
  if(ring_insert(sk) == -1)//在环形缓冲中插入上述sock结构sk
    goto free_pfr;
 ...
 */
static int packet_ring_bind(struct sock *sk, char *dev_name)//由ring_bind调用，用来为PF_RING分配环状缓存
{
  struct pf_ring_socket *pfr = ring_sk(sk);
		     ...
		     struct socket *sock, struct vm_area_struct *vma)
{
  ...
  /* Trick for mapping packet memory chunks *///mem_id=0
  if(mem_id >= 100) {
    mem_id -= 100;
